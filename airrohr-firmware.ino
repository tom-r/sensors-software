...
static void fetchSensorSDS(String& s) {
	...
	if (send_now) {
		...
		if (sds_val_count > 0) {
			...
			last_value_SDS_P1 = ExpSmoothVal(readCorrectionOffset(cfg::alphaSDS),float(float(sds_pm10_sum) / (sds_val_count * 10.0f)),last_value_SDS_P1,-1.0);
			last_value_SDS_P2 = ExpSmoothVal(readCorrectionOffset(cfg::alphaSDS),float(float(sds_pm25_sum) / (sds_val_count * 10.0f)),last_value_SDS_P2,-1.0);
...
static double ExpSmoothVal(float alpha,float newval,float oldSmoothedValue,float nanval)
{
